...
render_toc <- function(
  ...
  x <- paste(x, collapse = "\n")
  x <- paste0("\n", x, "\n")
  # This excludes any code snippets
  for (i in 5:3) {
  ...
  x <- sapply(x, function(h) {
    ...
    if (grepl("\\{#.+\\}(\\s+)?$", h)) {
      ...
      # SAO: Now my edits to remove all numbers at beginning and any special character except the hyphen 
      header_slug <- gsub("[^[:alnum:]-]", "", header_slug) 
      header_slug <- gsub("^\\d*", "", header_slug) 
      header_slug <- gsub("^-*", "", header_slug) 
    }
